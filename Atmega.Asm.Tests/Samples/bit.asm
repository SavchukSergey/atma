...
LSL R11	;Логический сдвиг влево	R(n+1) <- R(n), R(0) <- 0, C <- R(7)	S, Z, C, N, V, H	1
LSR r12	;Логический сдвиг вправо	R(n) <- R(n+1), R(7) <- 0, C <- R(0)	S, Z, C, N, V	1
ROL r31	;Сдвиг влево через перенос	R(n+1) <- R(n), R(0) <- C, C <- R(7)	S, Z, C, N, V, H	1
ROR r0	;Сдвиг вправо через перенос	R(n) <- R(n+1), R(7) <- C, C <- R(0)	S, Z, C, N, V	1
ASR r1	;Арифметический сдвиг вправо	R(n) <- R(n+1), R(7) <- R(7)	Z, C, N, V	1
BSET 2	;Установить бит в SREG	SREG(b) <- 1	SREG(b)	1
BCLR 3	;Сбросить бит в SREG	SREG(b) <- 0	SREG(b)	1
