...
JMP $	;Абсолютный переход	PC <- K	-	3
CALL $	;Абсолютный вызов	PC <- K	-	4 (5)
RJMP $	;Относительный переход	PC <- PC + K + 1	-	2
RCALL $	;Относительный вызов	PC <- PC + K + 1	-	2
...
CPSE R12,R22	;Сравнить и пропустить, если равно	if (Rn = Rk) PC <- PC + 2 or 3	-	1 / 2 / 3
